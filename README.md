...
- 🎯 Entry points to execute your program using `python -m <fastbackend>` or `$ fastbackend` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/pradeepdhawan/fastbackend/branch/main/graph/badge.svg?token=fastbackend_token_here)](https://codecov.io/gh/pradeepdhawan/fastbackend)
[![CI](https://github.com/pradeepdhawan/fastbackend/actions/workflows/main.yml/badge.svg)](https://github.com/pradeepdhawan/fastbackend/actions/workflows/main.yml)

Awesome fastbackend created by pradeepdhawan

## Install

from source
```bash
git clone https://github.com/pradeepdhawan/fastbackend fastbackend
cd fastbackend
make install
```
...
```bash
pip install fastbackend
```

## Executing

```bash
$ fastbackend run --port 8080
```

or

```bash
python -m fastbackend run --port 8080
```

or

```bash
$ uvicorn fastbackend:app
```
...
```bash
❯ fastbackend --help
Usage: fastbackend [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ fastbackend create-user --help
Usage: fastbackend create-user [OPTIONS] USERNAME PASSWORD
...
```bash
fastbackend create-user admin admin --superuser
```
...
Run with `fastbackend run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `fastbackend create-user`.
...
-----------------------------------------------------
fastbackend/__init__.py              4      0   100%
fastbackend/app.py                  16      1    94%
fastbackend/cli.py                  21      0   100%
fastbackend/config.py                5      0   100%
fastbackend/db.py                   10      0   100%
fastbackend/models/__init__.py       0      0   100%
fastbackend/models/content.py       47      1    98%
fastbackend/routes/__init__.py      11      0   100%
fastbackend/routes/content.py       52     25    52%
fastbackend/routes/security.py      15      1    93%
fastbackend/routes/user.py          52     26    50%
fastbackend/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
```py
from fastbackend.config import settings
```
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in fastbackend/default.toml.

### As environment variables
```bash
export fastbackend_KEY=value
export fastbackend_KEY="@int 42"
export fastbackend_KEY="@jinja {{ this.db.uri }}"
export fastbackend_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
...
```bash
fastbackend_ENV=production fastbackend run
```
